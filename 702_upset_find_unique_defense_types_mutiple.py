...
def find_unique_defense_types(input_file, output_file, locations, classifications, exclude_pdc):
    # 读取文件
    ...
    for loc, cls in single_conditions:
        filtered_df = df[(df['Location_BAF'] == loc) & (df['Contig_Classification'] == cls)]
        for dt in filtered_df['Defense_Type'].unique():
            if dt not in defense_type_conditions:
                defense_type_conditions[dt] = set()
            defense_type_conditions[dt].add((loc, cls))
    ...
    if not unique_defense_types:
        ...
        for comb, dt_set in unique_defense_types.items():
            ...
            for dt in dt_set:
                ...
                for loc, cls in comb:
                    count = df[(df['Defense_Type'] == dt) & (df['Location_BAF'] == loc) & (df['Contig_Classification'] == cls)].shape[0]
                    print(f"    Location_BAF: {loc}, Contig_Classification: {cls}, Count: {count}")
            ...
            with open(output_file, 'w') as f:
                for comb, dt_set in unique_defense_types.items():
                    ...
                    for dt in dt_set:
                        ...
                        for loc, cls in comb:
                            count = df[(df['Defense_Type'] == dt) & (df['Location_BAF'] == loc) & (df['Contig_Classification'] == cls)].shape[0]
                            f.write(f"    Location_BAF: {loc}, Contig_Classification: {cls}, Count: {count}\n")
                    f.write("\n")
...
if __name__ == "__main__":
    ...
    find_unique_defense_types(args.input, args.output, args.locations, args.classifications, args.exclude_pdc)
