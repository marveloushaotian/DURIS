...
data <- read.table("Collect/defense/new_test_2.txt", header = T, sep = "\t", stringsAsFactors = FALSE)
...
# Merge the metadata 'LocGroup1' column back into the filtered data
final_data <- cbind(LocGroup1 = metadata_ordered$LocGroup1, data_filtered)

# Now perform PCA on 'final_data' excluding the 'LocGroup1' column
...
pca_df <- data.frame(PC1 = pca_scores[,1], PC2 = pca_scores[,2], LocGroup1 = final_data$LocGroup1)

# Plot PCA results
ggplot(pca_df, aes(x = PC1, y = PC2, color = LocGroup1)) +
  geom_point() +
  theme_minimal() +
  labs(title = "PCA by LocGroup1", x = "Principal Component 1", y = "Principal Component 2")
...
# Read in the metadata and data files
metadata <- read.table("Collect/MetaData/Metadata_R_input.txt", header = TRUE, sep = "\t", stringsAsFactors = FALSE)
data <- read.table("Collect/defense/new_test_2.txt", header = TRUE, sep = "\t", check.names = FALSE)
...
# Read in the metadata and data files
metadata <- read.csv("Collect/overview/plasmidome_circular_padloc_matrix.csv")
data <- read.table("Collect/defense/non_redundant_defense_abundance_merged_final_right.txt", header = TRUE, sep = "\t", check.names = FALSE)
...
# Statistical analysis: PERMANOVA
permanova_result <- adonis2(final_data[,-1] ~ final_data[, "LocGroup1"], method = "bray")
p_value <- permanova_result$aov.tab$`Pr(>F)`[1]
...
# Proceed with the plot, checking if p_value_rounded is NA
plot_title <- if(is.na(p_value_rounded)) {
  "PCA by LocGroup1 - PERMANOVA p-value not available"
} else {
  paste("PCA by LocGroup1 - PERMANOVA p-value:", p_value_rounded)
}
...
ggplot(pca_df, aes(x = PC1, y = PC2, color = LocGroup1)) +
  ...
  stat_ellipse(type = "t", linetype = "dashed", alpha = 0.5) +
  geom_text_repel(
    data = aggregate(. ~ LocGroup1, pca_df, function(x) mean(range(x))),
    aes(label = LocGroup1),
    box.padding   = 0.5, 
    point.padding = 0.5,
    segment.color = 'grey50'
  ...
  theme_minimal() +
  labs(title = plot_title, x = "Principal Component 1", y = "Principal Component 2", color = "LocGroup1")

# Save the plot
ggsave("Results/04.defense variance/plasmidome_test_PCA_by_LocGroup1_with_group_names_vegan.pdf", width = 10, height = 6, dpi = 300)
```
...
# Read in the metadata and data files
metadata <- read.table("Collect/MetaData/Metadata_R_input.txt", header = TRUE, sep = "\t", stringsAsFactors = FALSE)
data <- read.csv("Collect/overview/plasmidome_circular_padloc_matrix.csv")
...
# Save the plot
ggsave("Results/04.defense variance/plasmidome_circular_PCA_by_LocGroup1_with_group_names_vegan.pdf", width = 6, height = 6, dpi = 300)
```
...
# Open PDF device
pdf('Results/04.defense variance/non_RDA_Plot.pdf', width = 11, height = 8)
...
abundance_data <- read.csv("Collect/overview/plasmidome_linear_padloc_matrix.csv")

metadata <- metadata[metadata$Sample %in% colnames(abundance_data), ]

abundance_data <- abundance_data %>% column_to_rownames("Defense_Name")
...
perform_anova <- function(df) {
  ...
  for (defense in defenses) {
    ...
    # Check if Country has more than one level
    if (length(unique(current_defense_data$Country)) > 1) {
      anova_result <- aov(Abundance ~ Country, data = current_defense_data)
...
# Plot
p <- ggplot(plot_data, aes(x = Country, y = Abundance, fill = Country)) +
  geom_boxplot() +
...
ggsave("Results/04.defense variance/plasmidome_linear_different_defense_Country.pdf", width = 30, height = 20, dpi = 300)
```
