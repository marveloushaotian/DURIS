...
Example usage: python batch_bwa_mem.py -i /path/to/fastq_dir -p prefix -o /path/to/output_dir""",
...
parser.add_argument('-t', '--threads', help='Number of threads for BWA. Example: -t 8', default='8', type=str)
parser.add_argument('--workers', help='Number of parallel workers. Example: --workers 4', default='4', type=int)
...
def run_bwa_mem(prefix, fastq1, fastq2, output_dir, threads):
    ...
    try:
        ...
        print(f"Successfully processed {fastq1} and {fastq2}")
    except subprocess.CalledProcessError as e:
        print(f"Error processing {fastq1} and {fastq2}: {e}")
...
if __name__ == '__main__':
    ...
    with ThreadPoolExecutor(max_workers=args.workers) as executor:
        future_to_pair = {executor.submit(run_bwa_mem, args.prefix, fastq1, fastq2, bwa_output_dir, args.threads): (fastq1, fastq2) for fastq1, fastq2 in fastq_pairs}
        
        for future in tqdm(as_completed(future_to_pair), total=len(future_to_pair), desc="Processing fastq file pairs"):
            fastq1, fastq2 = future_to_pair[future]
            try:
                future.result()
