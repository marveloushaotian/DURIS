...
for (defense_type in unique(data$Defense_Type)) {
  ...
  plots <- lapply(correlation_columns, function(col) {
    ...
    if (nrow(non_zero_data) > 5) {
      cor_test <- tryCatch({
        cor.test(non_zero_data[[col]], non_zero_data$GCGB)
      }, error = function(e) {
        return(list(p.value = NA))
      })
      
      if (!is.na(cor_test$p.value) && cor_test$p.value < 0.05) {
        create_scatter_plot(non_zero_data, col, "GCGB", defense_type)
      } else {
        NULL
      }
    } else {
  ...
  if (length(plots) > 0) {
    ...
    pdf(file.path("Results/03_DF_Cor_Everything/test7", filename), width = 24, height = 8 * n_rows)
