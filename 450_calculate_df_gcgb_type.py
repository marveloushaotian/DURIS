...
grouped_defense = df.groupby(['Contig_Classification', 'Country', 'Location', 'Defense_Type']).agg({
    'Defense_Number': 'sum'
}).reset_index()

# Group by Contig_Classification, Location, and Country to get unique Contig_Length
grouped_contigs = unique_contigs.groupby(['Contig_Classification', 'Country', 'Location']).agg({
    'Contig_Length': 'sum'
}).reset_index()

# Merge the dataframes to align defense sums with contig lengths
merged_df = pd.merge(grouped_defense, grouped_contigs, on=['Contig_Classification', 'Country', 'Location'])
...
merged_df.columns = ['Contig_Classification', 'Country', 'Location', 'Defense_Type', 'Total_Defense_Num', 'Total_Contig_Length', 'GCGB']
