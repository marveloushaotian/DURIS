...
doit(){
    mkdir -p Process/01.Defense/${2}/logs
    NAME=$(echo $1 | sed 's|.*\/||;s|\.fasta||')
    mkdir -p Process/01.Defense/${2}
    padloc --fna ${1} --outdir Process/01.Defense/${2}/ --fix-prodigal &> Process/01.Defense/${2}/logs/${NAME}.log
}

export -f doit

echo "Starting defense annotation with PADLOC for plasmids contigs..."
